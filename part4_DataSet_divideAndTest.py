...
# 使用支持向量机
model = svm.SVC()
model.fit(normDataSet[0: trainSize, :], datingLabels[0: trainSize])
...
for i in range(testSize):
    ...
    # result = K.knn(normDataSet[trainSize+i, :], normDataSet[0: trainSize, :], datingLabels[0: trainSize], k)

    # 使用支持向量机 , 这里要注意  需要通过reshape函数把数据进行规整
    result = model.predict(normDataSet[trainSize+i, :].reshape(1, -1))
